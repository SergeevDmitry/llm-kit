...
describe('no network calls, ever (README privacy claim)', () => {
  it('src/ never calls fetch/XMLHttpRequest and never imports node:http(s) or a provider SDK', () => {
    const forbiddenCallPatterns = [/\bfetch\s*\(/, /new\s+XMLHttpRequest\s*\(/];
    const forbiddenImportPatterns = [/^node:https?$/, /^@anthropic-ai\//, /^openai$/];
