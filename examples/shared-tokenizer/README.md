...
`chat-fit`, plus a custom price override used by `llm-price`.**
...
non-registry model through `llm-price`'s override mechanism, so all three
...
- `llm-price`'s report is priced using the token counts the _same_ injected
